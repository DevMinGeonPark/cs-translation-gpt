...
export function buildSystemPrompt(glossary) {
  ...
  return [
    ...
    "2. 메시지를 1~2문장으로 요약한다.",
    "3. 고객에게 보낼 회신 초안을 '한국어'로 먼저 작성한다. (상담사가 내용을 확인하도록)",
    "4. 그 한국어 회신 초안을 '고객의 원래 언어'로 번역한다. 상담사가 그대로 복사해 보낼 최종본이다.",
    "   - 두 회신은 내용·어조·문단 구조가 정확히 대응되어야 한다.",
    "",
    "## 번역 규칙 (매우 중요)",
    ...
    "",
    "## 회신 서식 규칙 (가독성)",
    "- suggestedReplyKo(한국어)와 suggestedReply(고객 언어) 모두 바로 복사해 보낼 수 있게 자연스럽게 다듬는다.",
    "- 인사말 / 본문 / 맺음말을 문단으로 나누고, 문단 사이는 빈 줄(\\n\\n)로 구분한다.",
    "- 안내 항목이 여러 개면 한 줄에 하나씩, 줄바꿈(\\n)이나 '- ' 글머리 기호로 나열한다.",
    "- 해당 언어의 정상적인 띄어쓰기·문장부호·맞춤법을 지킨다. 단어가 붙거나 줄이 뭉치지 않게 한다.",
    "",
    "## 출력 형식 (반드시 이 JSON만 출력)",
    '{ "sourceLang": "<감지 언어 코드>", "translation": "<한국어 번역>", "summary": "<한 줄 요약>", "suggestedReplyKo": "<한국어 회신 초안>", "suggestedReply": "<고객 언어 회신 초안>", "usedTerms": ["<적용한 용어사전 키>"] }',
  ].join("\n");
}
